...
export default function CompanyPage() {
  ...
  const [registrationCountry, setRegistrationCountry] = useState(startup?.registration_country ?? '');
  ...
  const [cofoundersOnDocs, setCofoundersOnDocs] = useState(startup?.cofounders_on_docs_confirmed ?? false);

  const [activeUsers, setActiveUsers] = useState(startup?.active_users != null ? String(startup.active_users) : '');
  const [monthlyRevenue, setMonthlyRevenue] = useState(startup?.monthly_revenue != null ? String(startup.monthly_revenue) : '');
  const [priorFunding, setPriorFunding] = useState(startup?.prior_funding_raised != null ? String(startup.prior_funding_raised) : '0');

  const [bankName, setBankName] = useState(startup?.bank_name ?? '');
  const [bankAccountName, setBankAccountName] = useState(startup?.bank_account_name ?? '');
  const [bankAccountNumber, setBankAccountNumber] = useState(startup?.bank_account_number ?? '');
  const [bankCurrency, setBankCurrency] = useState(startup?.bank_currency ?? '');
  const [bankProofUrl, setBankProofUrl] = useState(startup?.bank_proof_url ?? null);
