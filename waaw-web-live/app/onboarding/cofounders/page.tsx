...
interface CofounderDraft {
  dob: string; phone: string; nationality: string; address: string; previousAddress: string;
  currentCity: string; stateOfOrigin: string; stateOfResidence: string; postcode: string;
  educationLevel: string; educationInstitution: string; idType: string; idNumber: string; socialLink: string;
  selfieUrl: string | null; idDocUrl: string | null; idDocBackUrl: string | null;
  workHistory: WorkHistoryEntry[]; references: ReferenceEntry[];
}
...
function VerifyForm({ cofounder, onDone }: { cofounder: CofounderRow; onDone: () => void }) {
  ...
  const draftKey = `cofounder-${cofounder.id}`;
  const draft = useRef(loadDraft<CofounderDraft>(draftKey)).current;

  const [dob, setDob] = useState(cofounder.date_of_birth ?? draft.dob ?? '');
  const [phone, setPhone] = useState(cofounder.phone ?? draft.phone ?? '');
  const [nationality, setNationality] = useState(cofounder.nationality ?? draft.nationality ?? '');
  const [address, setAddress] = useState(cofounder.address_line ?? draft.address ?? '');
  const [previousAddress, setPreviousAddress] = useState(cofounder.previous_address ?? draft.previousAddress ?? '');
  const [currentCity, setCurrentCity] = useState(cofounder.current_city ?? draft.currentCity ?? '');
  const [stateOfOrigin, setStateOfOrigin] = useState(cofounder.state_of_origin ?? draft.stateOfOrigin ?? '');
  const [stateOfResidence, setStateOfResidence] = useState(cofounder.state_of_residence ?? draft.stateOfResidence ?? '');
  const [postcode, setPostcode] = useState(cofounder.postcode ?? draft.postcode ?? '');
  const [educationLevel, setEducationLevel] = useState(cofounder.education_level ?? draft.educationLevel ?? '');
  const [educationInstitution, setEducationInstitution] = useState(cofounder.education_institution ?? draft.educationInstitution ?? '');
  const [idType, setIdType] = useState(cofounder.id_type ?? draft.idType ?? '');
  const [idNumber, setIdNumber] = useState(cofounder.id_number ?? draft.idNumber ?? '');
  const [socialLink, setSocialLink] = useState(cofounder.social_link ?? draft.socialLink ?? '');
  const [selfieUrl, setSelfieUrl] = useState(cofounder.selfie_url ?? draft.selfieUrl ?? null);
  const [idDocUrl, setIdDocUrl] = useState(cofounder.id_document_url ?? draft.idDocUrl ?? null);
  const [idDocBackUrl, setIdDocBackUrl] = useState(cofounder.id_document_back_url ?? draft.idDocBackUrl ?? null);
  const [workHistory, setWorkHistory] = useState<WorkHistoryEntry[]>(cofounder.work_history?.length ? cofounder.work_history : draft.workHistory ?? []);
  const [references, setReferences] = useState<ReferenceEntry[]>(cofounder.reference_list?.length ? cofounder.reference_list : draft.references ?? []);
  ...
  useEffect(() => {
    saveDraft(draftKey, {
      dob, phone, nationality, address, previousAddress, currentCity, stateOfOrigin, stateOfResidence, postcode,
      educationLevel, educationInstitution, idType, idNumber, socialLink, selfieUrl, idDocUrl, idDocBackUrl,
      workHistory, references,
    });
