...
// back to (e.g. this page was opened directly from a bookmark or a new tab).
export function BackButton({ fallbackHref, label = 'Back' }: { fallbackHref: string; label?: string }) {
  ...
  return (
    <button
      ...
      className="mb-6 inline-block font-mono text-xs uppercase tracking-wider text-mu hover:text-pu"
    >
      ← {label}
    </button>
  );
...
export function Divider() {
  return <div className="my-5 h-px bg-ln" />;
}
